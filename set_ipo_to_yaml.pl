...
my $updated = 0;
my $msg = "\"Update ipo number ";

foreach my $block (sort keys %block_hash) {
    my $ipo = $block_hash{$block};
    if ( !$ps->IsBlock(-project => $chip, -rev => $rev, -view => $view, -block => $block) ) {
        die "Error: -block $block is not a valid block\n";
    }
    
    my $exist_ipo = "";
    if ($ps->HasBlock(-project => $chip, -rev => $rev, -view => $view, -block => $block, -param => "latest_ipo") ) {
        $exist_ipo = $ps->GetBlock(-project => $chip, -rev => $rev, -view => $view, -block => $block, -param => "latest_ipo");
    }
    
    if ($exist_ipo eq $ipo) {
        print "Info: in project $chip layout rev $rev, the latest ipo number of $block is the same as $ipo. skipping update\n";
        next;
    }
    
    print "Info: in project $chip layout rev $rev, update ipo number of $block from $exist_ipo to $ipo\n";
    $msg .= " $block from $exist_ipo to $ipo ";
    
    $ps->SetBlock(-project => $chip, -rev => $rev, -view => $view, -block => $block, -param => "latest_ipo", -value => $ipo);
...
if ($updated > 0) {
    ...
    $ps->Save();
    # -----------------------------------------
    open (OUT, ">$chip_root/timing/update_block_ipo");
    print OUT "p4_submit -m $msg $file\n";
    printf ("Info: Type \"source $chip_root/timing/update_block_ipo\" in this shell to check in the changes\n");
    close OUT;
    
    system("chmod 777 $chip_root/timing/update_block_ipo");
...
%corners = $ps->GetProject(-project => $chip, -param => "phases")
